...
class BinaryTree:
    ...
    This version with filename binaryTreeWED.py
    includes an additional method printexpWED,
    (WED = With Error Detection) that provides an
    expression error detection for improperly
    formatted expressions. printexpWED instead of
    printing, concatentating to the printline,
    prints to a temp file, that is created, opened,
    closed, and processed by buildParseTreeWED.py
    and supports the error detection that is added
    to that file.
    """
    ...
    # prints expression with parenthesis; uses inorder traversal
    # With Error Detection. Prints to file to support external function for error detection.
    def printexpWED(self, outfile):
        
        if self.leftChild:              
            print('(', end=' ', file=outfile)    # prints left parenthesis and space; if leaf skip            
            self.leftChild.printexpWED(outfile)  # recurs to print left child
        
        print(self.key, end=' ', file=outfile)   # prints root inorder and space; prints on recur        
        
        if self.rightChild:          
            self.rightChild.printexpWED(outfile) # recurs to print right child 
            print(')', end=' ', file=outfile)    # prints right parenthesis and space; if leaf skip
